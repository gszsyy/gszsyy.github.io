...
SLIDES = [
    (
        ...
        [
            ...
            "状态：创维 1600 已完成基础闭环验证；真机、SDK、Demo API 和端侧鲁棒性约束已记录。",
        ],
    ),
    (
        "阶段 1 目标",
    ...
    (
        ...
        [
            ...
            "该路线面向反光、弱暗光、运动模糊、极端角度、快速尺度变化和遮挡等传统方案易失效工况。",
        ],
    ),
    (
        "已完成事项",
    ...
    (
        "README 进度答复",
        [
            "Q1：创维眼镜真机是硬依赖；相机标定、性能调优、端侧推理帧率和 30 分钟稳定性测试必须在真机完成。",
            "中数元宇提供创维眼镜固件版本、SXR_SDK、修改后的高通 SDK、灰度摄像头调用和坐标转换方法。",
            "光速侧提供游戏 Demo、明确 Demo 形式，并暴露驱动击剑物体的 API。",
        ],
    ),
    (
        "当前限制",
    ...
    (
        "Q2 图像数据与标定",
        [
            "头显图像数据是算法精度与实时性的关键输入，由中数侧随 SDK 和文档提供。",
            "原始帧获取：取流 API / 回调、分辨率、帧率、像素格式、是否零拷贝。",
            "标定参数：相机内参、畸变模型及系数、相机相对头显/显示坐标系的外参。",
            "同步信息：帧时间戳、曝光/增益可读性、IMU 可用性。",
        ],
    ),
    (
        "Q3 Tag 选型",
        [
            "Tag 尺寸参考 20~40mm；近距小尺寸、远距大尺寸，可多尺寸混用。",
            "最终尺寸通过光照、运动、尺度变化、距离等条件交叉验证确定。",
            "材质参考标定板方案，需抗反光、漫反射、可贴合、耐磨耐汗。",
            "剑体和护具采用多枚冗余分布，多 Tag 按刚体分组并分配 ID 段。",
        ],
    ),
    (
        "当前交付物",
