...
SLIDES = [
    (
        ...
        [
            ...
            "状态：创维 1600 已完成基础驱动验证；底层取流、IMU 同步和端侧鲁棒性仍是下一步重点。",
        ],
    ),
    (
        "阶段 1 目标",
        [
            ...
            "通过多 Tag 融合得到 xyz 平移与旋转，驱动 Unity 中的虚拟物体。",
        ],
    ),
    (
        "技术路线",
        [
            "创维 1600 灰度图像取流 -> DeepTag 检测 -> 角点亚像素回归 -> ID 解码。",
            "多面体刚体约束融合可见 Tag，PnP + 非线性重投影精修输出 6DoF。",
            "路线已从传统 AprilTag 推进到 DeepTag，以提升低光、运动模糊和遮挡鲁棒性。",
        ],
    ),
    (
        "已完成事项",
        [
            "已在创维 1600 上完成初步验证，可以进行基本 Unity 驱动。",
    ...
    (
        "流程图",
        [
            "创维 1600 取流 -> DeepTag 检测 -> 多 Tag 融合 -> PnP 位姿 -> Unity 驱动。",
            "每个环节输出可观测 QC：检测置信度、角点残差、重投影误差、帧延迟和位姿抖动。",
        ],
    ),
    (
        "拓扑与知识图谱",
        [
            "实体多面体：八方向可见面、Tag ID、每面外参。",
            "设备侧：创维 1600、相机内参、畸变参数、灰度取流、时间戳。",
            "算法侧：DeepTag、角点回归、PnP、非线性优化、Unity 坐标系转换。",
        ],
    ),
    (
        "下一步计划",
        [
            "接入创维底层取流、时间戳和 IMU 同步接口。",
            "对 STL/STP 结构建立每面 Tag 坐标系和刚体外参。",
            "补充低光、运动模糊、反光和遮挡样本，完成 DeepTag 端侧压缩。",
            "建立 Unity 日志，量化延迟、丢帧、重投影误差和 6DoF 抖动。",
        ],
    ),
    (
        "验收建议",
        [
            ...
            "输出检测帧率、漏检率、位姿误差、重投影误差和 Unity 驱动延迟。",
        ],
    ),
]
